...
/**
 * Api/ScreencaptureInterval Controller
 *
 *
 * @method \App\Model\Entity\Api/ScreencaptureInterval[]|\Cake\Datasource\ResultSetInterface paginate($object = null, array $settings = [])
 */
class ScreencaptureIntervalController extends AppController
{
    ...
    /**
     * @api {post} /api/screencapture_interval.json Assign screen capture interval for staff
     * @apiName Assign screen capture interval for staff
     * @apiDescription Assign screen capture idle for staff.
     *
     ...
     * @apiParam {String} interval_value Screen Capture Interval (expected value in seconds) **Required Field**
    ...
    public function add()
    {
        ...
        if ($entity) {
          $dataLog = [
              ...
              'remarks' => "Updated Screen Capture Time in Workbench Settings",
              ...
              'payload' => serialize(json_encode(['screen_capture' => $this->request->getData('interval_value')])),
              'prev_payload' => serialize(json_encode(['screen_capture' => $entity->screen_capture])),
              'affected_user' => $this->staffEntity->username,
              'affected_user_fullname' => $this->staffEntity->first_name . $this->staffEntity->last_name,
          ];
          $workbenchSettingsEntity = $this->workbenchSettingsTable->patchEntity($entity, ['screen_capture' => $this->request->getData('interval_value')]);
        } else {
          $data = [
            'staff_id' => $this->staffEntity->id,
            'client_id' => $this->clientEntity->id,
            'screen_capture' => $this->request->getData('interval_value'),
          ];
          $workbenchSettingsEntity = $this->workbenchSettingsTable->newEntity($data);
          $dataLog = [
              'path' => $this->request->getRequestTarget(),
              'remarks' => "Added new Screen Capture Time in Workbench Settings",
              'performed_by' => $this->request->getData('performed_by'),
              'performed_by_fullname' => $this->request->getData('performed_by_fullname'),
